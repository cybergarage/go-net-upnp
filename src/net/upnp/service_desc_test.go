...
func checkTestServiceDescription(t *testing.T, srv *Service) {
	...
	for n, stateVar := range srv.ServiceStateTable.StateVariables {
...
func TestServiceLoadDescription(t *testing.T) {
	...
	checkTestServiceDescription(t, srv)

	// Check output description

	outDesc, err := srv.DescriptionString()
	if err != nil {
		t.Error(err)
	}
	...
	checkTestServiceDescription(t, outSrv)
}
